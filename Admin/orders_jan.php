  ...
  <div class="container-fluid p-3">
    <div class="row">
      ...
      <div class="col">
        ...
        $total = mysqli_query($connection, "select sum(`total_amount`) AS total_amount from orders WHERE date BETWEEN '2022-01-01' AND '2022-01-30'");
        ...
        ?>
          <table class="table">
            ...
            <tbody>
              <?php
              $orders = mysqli_query($connection, "select * from orders WHERE date BETWEEN '2022-01-01' AND '2022-01-30' order by date desc");
              $total = mysqli_query($connection, "select sum(`total_amount`) AS total_amount from orders WHERE date BETWEEN '2022-01-01' AND '2022-01-30'");
              $row=mysqli_fetch_array($total);
              $total_amount=$row['total_amount'];
              ...
              while ($order = mysqli_fetch_array($orders)) {
                ...
                if ($order['status'] == 'pending') {
                  echo "<td><a class='btn btn-success btn-sm' href='orders_jan.php?action=update&order_id={$order['order_id']}'>Complete</a></td>";
                } else {
